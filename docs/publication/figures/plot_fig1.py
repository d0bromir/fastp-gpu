...
def main():
    ...
    for ds_key, ds_label in DS_ORDER:
        ...
        if not cpu_row and not gpu_row:
            continue  # no stage-breakdown data for this dataset; omit entirely
                      # rather than reserving a tick label with no bars (was
                      # colliding with the previous category's label)
