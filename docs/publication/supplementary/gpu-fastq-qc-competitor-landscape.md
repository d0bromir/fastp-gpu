...
search pass, see §1.6 and §5).

---

## 1. Summary of corrections to the proposed table

1. **RabbitQCPlus is not GPU-accelerated**, despite marketing language about an
   "integrated CARE error correction engine." Verified: the repository contains
   **zero `.cu` files**, and its `Makefile` hard-codes
   `THRUST_DEVICE_SYSTEM_OMP` (Thrust's OpenMP backend) for every build
   configuration. GitHub's "10% Cuda" language statistic reflects vendored
   Thrust headers, not compiled device code. RabbitQCPlus is a highly
   AVX-512/AVX2-vectorized **CPU** tool. It remains a legitimate *CPU* baseline
   competitor, but must not be described as GPU-accelerated.
...
- **RabbitQCPlus GPU claim:** searched the GitHub repository directly for
