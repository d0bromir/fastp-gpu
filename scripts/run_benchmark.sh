...
PREFLIGHT_LOG="$RESULTS_DIR/full_preflight_${TIMESTAMP}.log"

OPENGENE="$PROJ_DIR/../fastp_opengene/fastp"
CPU_BIN="$PROJ_DIR/fastp-cpu"
GPU_BIN="$PROJ_DIR/fastp"
OG_CSV=""
# Set to 1 via --cpu-only to skip the GPU build, GPU binary checks, and GPU
# benchmark runs entirely.  Use on machines without a CUDA-capable GPU.
CPU_ONLY=0
...
# Set to 0 to disable.
# 3600s (60 min): covers large datasets at T=1/T=2 (e.g. WGS_PE_40G opengene
# T=1 ~3188s, T=2 ~2744s; BS_HiSeq_SE_26.8G d0bromir T=1 ~2500s).
...
HANG_GRACE_SECS=${HANG_GRACE_SECS:-60}   # ignore first N secs (I/O-heavy start)
HANG_IDLE_SECS=${HANG_IDLE_SECS:-300}    # how long low-CPU must persist (5 min)
HANG_CPU_PCT=${HANG_CPU_PCT:-2}          # % threshold below which = idle
...
HANG_OUTPUT_IDLE_SECS=${HANG_OUTPUT_IDLE_SECS:-600}
...
# Maximum allowed percent difference between d0bromir and opengene compressed
# output file sizes (either direction).  Enforced only at VALIDATE_THREADS.
# A larger compressed output means more I/O; a smaller one means less.  Either
# direction gives an unfair wall-time advantage vs the baseline.  Default 10%
# gives comfortable headroom for libdeflate L3 vs opengene zlib L4 (~+3%).
# Hard-fails the benchmark (exit 2) when exceeded.
# Override: MAX_COMPRESSED_SIZE_DELTA_PCT=N
MAX_COMPRESSED_SIZE_DELTA_PCT=${MAX_COMPRESSED_SIZE_DELTA_PCT:-10}
...
usage() {
    ...
    echo "  --use-og-csv [FILE]  Reuse opengene baseline from FILE (skips opengene"
    echo "                       runs for entries present in the CSV; falls back"
    echo "                       to live opengene runs for any (dataset, threads)"
    echo "                       slot missing from the CSV)."
    ...
    echo "                       non-IDLE_TIMEOUT, non-FAIL row in that CSV."
    ...
    echo "  --cpu-only           Skip GPU build, GPU tests, and GPU benchmark runs."
    echo "                       Use on machines without a CUDA-capable GPU."
    ...
    case "$1" in
        ...
        --use-og-csv)
            ...
            if [[ $# -gt 0 && "${1:-}" != -* ]]; then
                ...
                OG_CSV="$RESULTS_ROOT/fastp-gpu_v1.3.3-d0bromir/vs_opengene_v1.3.3/20260515_132505/full_benchmark_20260515_132505.csv"
            fi
            ;;
        --validate-threads)
            shift
        ...
        --resume)
            shift
            # Optional file argument: use it if provided and not another flag.
            if [[ $# -gt 0 && "${1:0:1}" != "-" && -f "$1" ]]; then
                RESUME_CSV="$1"
                shift
            fi
            # If no file given, auto-detection runs via resolve_resume_csv().
            ;;
        --no-resume)
            NO_RESUME=1
            shift
...
if [[ "$SCENARIO" != "default" ]]; then
    ...
    PREFLIGHT_LOG="$RESULTS_DIR/full_preflight_${TIMESTAMP}_${SCENARIO}.log"
fi
...
# Stores opengene compressed output sizes (bytes) for size fairness check,
# only at VALIDATE_THREADS:  key = "ds"
declare -A OG_OUT1_BYTES OG_OUT2_BYTES
...
if [[ -n "$OG_CSV" ]]; then
    if [[ "$CPU_ONLY" == "1" ]]; then
        TOTAL_RUNS=$(( ${#DATASETS[@]} * 1 * ${#THREAD_COUNTS[@]} ))
    else
...
load_completed_runs() {
    ...
    while IFS=, read -r ds tp tool thr rep wall ri ro pct vl rest; do
        ...
        # TIMEOUT / IDLE_TIMEOUT / FAIL / CRASH runs are NOT completed and will retry.
        case "${pct},${vl}" in
...
if [[ -n "$RESUME_CSV" && -f "$RESUME_CSV" ]]; then
    ...
    PREFLIGHT_LOG="$OUT_DIR/full_preflight_${ts}.log"
    load_completed_runs
    printf "\n=== Resumed: %s ===\n\n" "$(date)" >> "$PROFILE_LOG"
    printf "\n=== Resumed: %s ===\n\n" "$(date)" >> "$VALID_LOG"
else
    ...
    (( RUN_NUM > progress )) && progress=$RUN_NUM
    {
        ...
        if [[ -s "$CSV" ]] && [[ $(wc -l < "$CSV") -gt 1 ]]; then
            ...
            tail -n +2 "$CSV" | while IFS=, read -r ds tp tool thr rep wt ri ro pct vl _rest; do
                # walltime may be 'N/A' for rows imported via --use-og-csv;
                # render as string in that case to avoid printf failing on
                # a non-numeric value passed to %f.
                if [[ "$wt" =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
...
# OG_LIVE_NEEDED=1 means the user asked --use-og-csv but the CSV is missing
# one or more (dataset, threads) baselines, so we will run opengene live
# for those slots and use the CSV for the rest.
OG_LIVE_NEEDED=0
OG_MISSING_KEYS=()
if [[ -n "$OG_CSV" ]]; then
    if [[ ! -f "$OG_CSV" ]]; then
    ...
    # Detect baselines that are missing for the requested (dataset, threads)
    # matrix.  Any miss flips OG_LIVE_NEEDED so we will rebuild the opengene
    # binary and run it live for those slots.
    for _ds in "${DATASETS[@]}"; do
        for _thr in "${THREAD_COUNTS[@]}"; do
            _w="${WALL_TIMES[${_ds}:opengene:${_thr}]:-}"
            if [[ -z "$_w" || "$_w" == "N/A" ]]; then
                OG_LIVE_NEEDED=1
                OG_MISSING_KEYS+=("${_ds}@T=${_thr}")
            fi
        done
    done
    if [[ "$OG_LIVE_NEEDED" == "1" ]]; then
        echo "  --use-og-csv: ${#OG_MISSING_KEYS[@]} baseline(s) missing — will run opengene live for: ${OG_MISSING_KEYS[*]}"
    fi
    echo ""
fi
...
echo " CPU-only  : ${CPU_ONLY}  (GPU build/tests/runs skipped when 1)"
...
if [[ "${SKIP_PREFLIGHT_BUILD:-0}" != "1" ]]; then
    ...
    if [[ "$CPU_ONLY" == "1" ]]; then
        # CPU-only mode: build opengene + d0bromir CPU; skip GPU entirely.
        if [[ -n "$OG_CSV" && "$OG_LIVE_NEEDED" == "0" ]]; then
            echo "  --cpu-only + --use-og-csv: rebuilding d0bromir CPU only"
            "$SCRIPT_DIR/build_all.sh" cpu \
                || { echo "  [FAIL] d0bromir CPU build failed — aborting benchmark." >&2; exit 2; }
        else
            echo "  --cpu-only: rebuilding opengene + d0bromir CPU (skipping GPU)"
            "$SCRIPT_DIR/build_all.sh" opengene \
                || { echo "  [FAIL] opengene build failed — aborting benchmark." >&2; exit 2; }
            "$SCRIPT_DIR/build_all.sh" cpu \
                || { echo "  [FAIL] d0bromir CPU build failed — aborting benchmark." >&2; exit 2; }
        fi
    elif [[ -n "$OG_CSV" && "$OG_LIVE_NEEDED" == "0" ]]; then
        # opengene baseline fully covered by CSV — rebuild only d0bromir.
    ...
    elif [[ -n "$OG_CSV" && "$OG_LIVE_NEEDED" == "1" ]]; then
        # Some baselines missing — must also build opengene for the live runs.
        echo "  --use-og-csv set but some baselines missing; rebuilding all three (opengene + d0bromir CPU + GPU)"
...
for pair in "opengene:$OPENGENE" "d0bromir_cpu:$CPU_BIN" "d0bromir_gpu:$GPU_BIN"; do
    lbl="${pair%%:*}"; pth="${pair##*:}"
    if [[ "$lbl" == "opengene" && -n "$OG_CSV" && "$OG_LIVE_NEEDED" == "0" ]]; then
        echo "  [SKIP] $lbl  (baseline loaded from CSV)"
        continue
    fi
    if [[ "$lbl" == "d0bromir_gpu" && "$CPU_ONLY" == "1" ]]; then
        echo "  [SKIP] $lbl  (--cpu-only)"
        continue
    fi
...
if [[ "${SKIP_PREFLIGHT_TESTS:-0}" != "1" ]]; then
    ...
    printf "=== Preflight Tests: %s ===\n\n" "$(date)" > "$PREFLIGHT_LOG"
    test_args=()
    # Skip opengene unit-tests only when we genuinely will not invoke the
    # opengene binary at all (CSV fully covers the baseline matrix).
    [[ -n "$OG_CSV" && "$OG_LIVE_NEEDED" == "0" ]] && test_args+=(--skip-opengene)
    # Skip GPU unit-tests and CPU<->GPU regression when --cpu-only is set.
    [[ "$CPU_ONLY" == "1" ]] && test_args+=(--skip-gpu)
    if ! OPENGENE_BIN="$OPENGENE" CPU_BIN="$CPU_BIN" GPU_BIN="$GPU_BIN" \
            "$SCRIPT_DIR/run_tests.sh" "${test_args[@]}" -q \
            2>&1 | tee -a "$PREFLIGHT_LOG"; then
        echo "  [FAIL] pre-flight tests failed — aborting benchmark." | tee -a "$PREFLIGHT_LOG" >&2
        exit 2
    fi
    printf "\n=== Preflight Tests PASSED: %s ===\n" "$(date)" >> "$PREFLIGHT_LOG"
    echo ""
fi
...
run_one() {
    ...
    (
        ...
        echo "[$(date +%H:%M:%S)] WATCHDOG START pid=$$ fastp=$fastp_pid hard=${RUN_TIMEOUT:-3600} cpu_idle=${HANG_IDLE_SECS:-30}s out_idle=${HANG_OUTPUT_IDLE_SECS:-180}s" > "$watchdog_log"

        grace=${HANG_GRACE_SECS:-20}
        idle_limit=${HANG_IDLE_SECS:-30}
        cpu_thresh=${HANG_CPU_PCT:-5}
        out_idle_limit=${HANG_OUTPUT_IDLE_SECS:-180}
        hard_limit=${RUN_TIMEOUT:-3600}
        ...
        while :; do
            ...
            if [[ "${hard_limit:-0}" -gt 0 && "$elapsed" -ge "$hard_limit" ]]; then
                ...
                # Write sentinel BEFORE killing so the parent sees it even if
                # it reads the file as soon as wait() returns on SIGTERM exit.
            ...
            if [[ -n "$stat_line" ]]; then
                ...
                if [[ "${prev_ts:-0}" -gt 0 ]]; then
                    ...
                    if [[ "$idle_streak" -ge "$idle_limit" ]]; then
                        ...
                        # Write sentinel BEFORE killing (see HARD_TIMEOUT note above).
    ...
    # exit_code 124 = timeout (SIGTERM from `timeout` cmd); 137 = killed (SIGKILL);
    # 143 = 128+15 = died from SIGTERM (watchdog sent TERM before writing sentinel).
    # CSV status:
    #   TIMEOUT      — hard wall-time limit expired (HARD_TIMEOUT sentinel or bare exit 124)
    #   IDLE_TIMEOUT — process stalled with no CPU / output progress (HANG_DETECTED sentinel)
    if [[ $exit_code -eq 124 || $exit_code -eq 137 || $exit_code -eq 143 ]]; then
        local run_status="TIMEOUT"
        local reason="TIMEOUT after ${wall_s}s (limit ${RUN_TIMEOUT}s)"
        if [[ "$watchdog_msg" == HANG_DETECTED* ]]; then
            run_status="IDLE_TIMEOUT"
            reason="IDLE_TIMEOUT after ${wall_s}s ($watchdog_msg)"
        elif [[ -n "$watchdog_msg" ]]; then
            reason="TIMEOUT after ${wall_s}s ($watchdog_msg)"
        fi
        echo "  [KILLED] ${tool_label} ${ds} T=${threads} — ${reason}" | tee -a "$VALID_LOG"
        printf "  [%s]\n" "$run_status"
        rm -f "$tmp_out1" "$tmp_stderr"
        [[ -n "$tmp_out2" ]] && rm -f "$tmp_out2"
        _RUN_WALL="$wall_s"; _RUN_RI=0; _RUN_RO=0; _RUN_PCT="$run_status"; _RUN_JSON="$tmp_json"
        _RUN_EXIT="$exit_code"
        return 0
    fi
...
# Watchdog kills (TIMEOUT/HANG) are retried ONCE.  When that happens the
# original TIMEOUT outcome is appended to the CSV as a history row
# (rep=timeout1) before the retry runs; the caller's normal write captures
# the retry's outcome.
# Sets _RUN_ATTEMPTS to the attempt number that succeeded (or MAX_ATTEMPTS
# on persistent failure).
# ---------------------------------------------------------------------------
_emit_timeout_history_row() {
    # Args: tool_label ds threads rep_marker
    # Reads _RUN_* globals captured at the moment of TIMEOUT.
    local tool_label="$1" ds="$2" threads="$3" rep_marker="$4"
    local ds_type="${DS_TYPE[$ds]:-PE}"
    local pct_col="${_RUN_PCT:-TIMEOUT}"
    # opengene rows use 'baseline'/'N/A' in column 9; d0bromir rows use the pct.
    if [[ "$tool_label" == "opengene" ]]; then
        pct_col="N/A"
    fi
    echo "${ds},${ds_type},${tool_label},${threads},${rep_marker},${_RUN_WALL:-0},${_RUN_RI:-0},${_RUN_RO:-0},${pct_col},${_RUN_PCT:-TIMEOUT},${_RUN_ATTEMPTS:-1},${_RUN_CPU_AVG:-0},${_RUN_CPU_PEAK:-0},${_RUN_RSS_PEAK:-0},${_RUN_GPU_AVG:-0},${_RUN_GPU_PEAK:-0},${_RUN_GPU_MEM_PEAK:-0},${SCENARIO}" >> "$CSV"
}
...
run_with_retries() {
    ...
    for (( attempt = 1; attempt <= MAX_ATTEMPTS; attempt++ )); do
        ...
        # TIMEOUT / HANG: append a history row to the CSV and retry once.
        if [[ "${_RUN_PCT:-}" == "TIMEOUT" ]]; then
            if [[ "$timeout_retried" -eq 0 ]]; then
                _emit_timeout_history_row "$tool_label" "$ds" "$threads" "timeout1"
                echo "  [RETRY] TIMEOUT recorded as history; retrying once ds=$ds tool=$tool_label T=$threads" \
                    | tee -a "$VALID_LOG"
                timeout_retried=1
                free_run_outputs
                sleep 5
            ...
            # Second timeout: give up; caller writes the final TIMEOUT row.
            return 0
        fi
...
validate_against_opengene() {
    ...
    if [[ -z "$og_ri" || -z "$og_ro" || "$og_ri" == "0" ]]; then
        echo "[SKIP] $tool_label $ds T=$threads — no valid opengene baseline (og timed out or missing)" | tee -a "$VALID_LOG"
        _VALID="SKIP"; return 0
    fi

    # If the opengene baseline at this thread count differs from the canonical
    # reference run (opengene at VALIDATE_THREADS), the baseline is internally
    # inconsistent — likely a race in opengene at high thread counts.  Use the
    # canonical reference so an opengene non-determinism bug does not cause a
    # false validation failure for d0bromir.
    local ref_ri ref_ro
    ref_ri="${OG_READS_IN[${ds}:${VALIDATE_THREADS}]:-}"
    ref_ro="${OG_READS_OUT[${ds}:${VALIDATE_THREADS}]:-}"
    if [[ -n "$ref_ri" && "$ref_ri" != "0" && "$ref_ri" != "$og_ri" ]]; then
        echo "[OG_INCONSISTENT] opengene $ds T=$threads reads_in=$og_ri reads_out=$og_ro" \
             "differs from T=$VALIDATE_THREADS canonical (reads_in=$ref_ri reads_out=$ref_ro)" \
             "— validating $tool_label against canonical ref" | tee -a "$VALID_LOG"
        og_ri="$ref_ri"
        og_ro="$ref_ro"
    fi
...
validate_full_against_opengene() {
    ...
    # --- Compressed output size fairness check ---
    local og_bytes_1="${OG_OUT1_BYTES[$ds]:-0}"
    local og_bytes_2="${OG_OUT2_BYTES[$ds]:-0}"
    if [[ "${og_bytes_1:-0}" -gt 0 && -f "$test_fq1" ]]; then
        local te_bytes_1
        te_bytes_1=$(stat -c %s "$test_fq1" 2>/dev/null || echo 0)
        local size_delta_r1
        size_delta_r1=$(awk -v og="$og_bytes_1" -v te="$te_bytes_1" \
            'BEGIN{ if (og>0) printf "%.1f", (te-og)/og*100; else print "N/A" }')
        echo "[SIZE] $tool_label $ds T=$threads R1 compressed: ${te_bytes_1}B vs opengene ${og_bytes_1}B  delta=${size_delta_r1}%" \
            | tee -a "$VALID_LOG"
        if awk -v d="${size_delta_r1}" -v lim="${MAX_COMPRESSED_SIZE_DELTA_PCT:-10}" \
            'BEGIN{ v=d+0; if(v<0)v=-v; exit !(v > lim+0) }'; then
            echo "[FAIL] $tool_label $ds T=$threads  R1 compressed size delta ${size_delta_r1}% exceeds \xc2\xb1${MAX_COMPRESSED_SIZE_DELTA_PCT}% limit" \
                | tee -a "$VALID_LOG"
            echo "BENCHMARK ABORTED — compressed output size fairness failure" | tee -a "$VALID_LOG"
            _VALID="FAIL"
            kill "$TICKER_PID" 2>/dev/null
            exit 2
        fi
    fi
    if [[ -n "$test_fq2" && "${og_bytes_2:-0}" -gt 0 && -f "$test_fq2" ]]; then
        local te_bytes_2
        te_bytes_2=$(stat -c %s "$test_fq2" 2>/dev/null || echo 0)
        local size_delta_r2
        size_delta_r2=$(awk -v og="$og_bytes_2" -v te="$te_bytes_2" \
            'BEGIN{ if (og>0) printf "%.1f", (te-og)/og*100; else print "N/A" }')
        echo "[SIZE] $tool_label $ds T=$threads R2 compressed: ${te_bytes_2}B vs opengene ${og_bytes_2}B  delta=${size_delta_r2}%" \
            | tee -a "$VALID_LOG"
        if awk -v d="${size_delta_r2}" -v lim="${MAX_COMPRESSED_SIZE_DELTA_PCT:-10}" \
            'BEGIN{ v=d+0; if(v<0)v=-v; exit !(v > lim+0) }'; then
            echo "[FAIL] $tool_label $ds T=$threads  R2 compressed size delta ${size_delta_r2}% exceeds \xc2\xb1${MAX_COMPRESSED_SIZE_DELTA_PCT}% limit" \
                | tee -a "$VALID_LOG"
            echo "BENCHMARK ABORTED — compressed output size fairness failure" | tee -a "$VALID_LOG"
            _VALID="FAIL"
            kill "$TICKER_PID" 2>/dev/null
...
for ds in "${DATASETS[@]}"; do
    ...
    for threads in "${THREAD_COUNTS[@]}"; do
        ...
        # Decide whether the opengene slot for (ds, threads) needs a
        # live run.  Reasons: no --use-og-csv at all, or --use-og-csv was
        # passed but the CSV had no baseline for this (ds, threads).
        _og_have_csv=0
        if [[ -n "$OG_CSV" ]]; then
            _w="${WALL_TIMES[${ds}:opengene:${threads}]:-}"
            if [[ -n "$_w" && "$_w" != "N/A" ]]; then
                _og_have_csv=1
            fi
        fi

        # Whether deep validation runs at this thread count.  Live opengene
        # runs (whether or not --use-og-csv is set) can produce reference
        # FASTQ/JSON, so deep validation activates when we actually run
        # opengene at T=VALIDATE_THREADS.
        is_validate_t=0
        if [[ "$threads" == "$VALIDATE_THREADS" && "$_og_have_csv" == "0" ]]; then
            is_validate_t=1
        fi

        # opengene
        if [[ "$_og_have_csv" == "0" ]]; then
            RUN_NUM=$(( RUN_NUM + 1 ))
            if [[ -n "${COMPLETED[${ds}:opengene:${threads}]:-}" ]]; then
                ...
                if [[ "$is_validate_t" == "1" && "$_og_ok" == "1" ]]; then
                    ...
                    OG_OUT1_BYTES["$ds"]=$(stat -c %s "$_RUN_OUT1" 2>/dev/null || echo 0)
                    [[ -n "$_RUN_OUT2" ]] && OG_OUT2_BYTES["$ds"]=$(stat -c %s "$_RUN_OUT2" 2>/dev/null || echo 0)
...
printf "%s\n" "$(printf -- '-%.0s' {1..105})"
tail -n +2 "$CSV" | while IFS=, read -r ds tp tool thr rep wt ri ro pct vl _rest; do
    # walltime may be 'N/A' for rows imported via --use-og-csv; only
    # apply numeric formatting when wt parses as a number.
    if [[ "$wt" =~ ^[0-9]+(\.[0-9]+)?$ ]]; then
        printf "%-22s  %-4s  %-15s  %5s  %10.1f  %12s  %12s  %12s  %s\n" \
            "$ds" "$tp" "$tool" "$thr" "$wt" "$ri" "$ro" "$pct" "$vl"
    else
