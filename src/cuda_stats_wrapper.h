...
private:
    ...
    //
    // Slot count is runtime-configurable via the FASTP_GPU_SLOTS env var
    // (clamped to [1, MAX_SLOTS]). The historical default of 8 was set when
    // -w 32 PE meant ~16 worker threads; at higher worker counts those
    // threads pile up on the slot-0 fallback path, so 16-32 slots can help
    // T>=16. Default 8 preserves existing behavior; canonical benchmarks
    // run unchanged unless FASTP_GPU_SLOTS is set explicitly.
    static const int MAX_SLOTS     = 32;
    static const int DEFAULT_SLOTS = 8;
    int              mNumSlots;
