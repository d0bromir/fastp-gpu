...
// With MAX_PACK_SIZE=8192 we need at least 8192; use 2× for headroom.
// Each slot's seq/qual packed buffer = SLOT_BUF_BYTES bytes.
// Total GPU memory per device = mNumSlots × 2 × (SLOT_BUF_BYTES + pointer/int
// arrays). At default 8 slots that is ~512 MB; at 32 slots ~2 GB.
// -----------------------------------------------------------------------
...
static int resolve_num_slots(int default_slots, int max_slots) {
    const char* env = std::getenv("FASTP_GPU_SLOTS");
    int n = default_slots;
    if (env && *env) {
        int v = std::atoi(env);
        if (v >= 1 && v <= max_slots) n = v;
    }
    return n;
}
...
CudaStatsWrapper::CudaStatsWrapper(int device)
    : gpu_available(false), device_id(-1),
      mNumSlots(resolve_num_slots(DEFAULT_SLOTS, MAX_SLOTS)), next_slot_hint(0)
{
#ifdef HAVE_CUDA
    ...
    if (gpu_available) {
        string msg = string("[GPU] CUDA GPU available (Device ") +
                     ...
                     " (slots=" + to_string(mNumSlots) + ")";
        loginfo(msg);

        slots.reset(new GpuSlot[mNumSlots]);
...
{
#ifdef HAVE_CUDA
    if (gpu_available && slots) {
        cudaSetDevice(device_id);
        for (int i = 0; i < mNumSlots; i++) freeSlot(slots[i]);
    }
#endif
}
...
// Public entry point.
//
// Acquires any free slot via try_lock (round-robin hint); if all mNumSlots
// slots are busy it blocks on slot 0.  This allows up to mNumSlots concurrent
    ...
    int trim_window_size)
{
    ...
    if (!gpu_available) {
        // CPU fallback: no CUDA device available at runtime.
        stats.clear();
    ...
    int hint = next_slot_hint.fetch_add(1, std::memory_order_relaxed) % mNumSlots;

    // Try non-blocking acquisition on all slots starting from hint
    for (int i = 0; i < mNumSlots; i++) {
        int idx = (hint + i) % mNumSlots;
    ...
    struct GpuBatchPostStats& batch_post_stats)
{
    ...
    if (!gpu_available) return -1;   // caller falls back to CPU path
