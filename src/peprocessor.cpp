...
PairEndProcessor::PairEndProcessor(Options* opt){
    ...
    mContaminantDB = NULL;
    if (mOptions->contaminant.enabled) {
        mContaminantDB = new ContaminantDB();
        mContaminantDB->build(mOptions);
    }
...
bool PairEndProcessor::processPairEnd(ReadPack* leftPack, ReadPack* rightPack, ThreadConfig* config){
    ...
    for(int p=0; p<pack_size; p++){
        ...
        if(!mergeProcessed) {
            ...
            // k-mer contamination scan: check R1 when both mates pass quality filter
            if (mContaminantDB && result1 == PASS_FILTER && result2 == PASS_FILTER
                    && r1 != NULL) {
                int srcId = mContaminantDB->scanRead(r1);
                if (srcId > 0)
                    config->getFilterResult()->addContamHit(srcId);
            }
...
void PairEndProcessor::readerTask(bool isLeft)
{
    ...
    while(true){
        ...
        if(count == mEffectivePackSize || needToBreak){
            ...
            // memory usage (threshold scales with thread count -- see packInMemLimit())
            if(isLeft) {
                while(mLeftPackReadCounter - mPackProcessedCounter > packInMemLimit(mEffectiveThreads)){
                    //cerr<<"sleep"<<endl;
                    slept++;
                    usleep(100);
                }
            } else {
                while(mRightPackReadCounter - mPackProcessedCounter > packInMemLimit(mEffectiveThreads)){
                    //cerr<<"sleep"<<endl;
                    slept++;
                    usleep(100);
                }
            ...
            if(readNum % (mEffectivePackSize * PACK_IN_MEM_LIMIT) == 0 && mLeftWriter) {
                while( (mLeftWriter && mLeftWriter->bufferLength() > packInMemLimit(mEffectiveThreads)) || (mRightWriter && mRightWriter->bufferLength() > packInMemLimit(mEffectiveThreads)) ){
                    slept++;
                    usleep(1000);
                }
            }
...
void PairEndProcessor::processorTask(ThreadConfig* config)
{
    ...
    while(true) {
        ...
        if(inputLeft->isProducerFinished() && !inputLeft->canBeConsumed()) {
            ...
            // Block on whichever queue is actually empty, instead of
            // busy-polling with usleep: at high -w this polling pattern
            // starves the CPU with wake/sleep churn across many worker
            // threads. waitForData() wakes immediately when the reader
            // produces (or finishes); the bounded timeout is just a
            // safety net, not the primary wake mechanism.
            if(!inputLeft->canBeConsumed())
                inputLeft->waitForData(500);
            else
                inputRight->waitForData(500);
        }
    }
