...
public:
    ...
    // Deferred GPU init — safe to call from any thread (mutex-protected, idempotent).
    // Exposed as public so processor classes can overlap init with I/O start-up.
    void ensureGPUInit();
...
private:
    ...
    std::future<void> mGpuInitFuture;      // Background init started in ctor
