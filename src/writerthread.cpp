...
static int numCompressors(Options* opt, int nThreads) {
    const char* env = getenv("FASTP_COMPRESSORS");
    if (env && *env) {
        int v = atoi(env);
        if (v > 0) return v;
    }
    int processorThreads = nThreads;
...
WriterThread::WriterThread(Options* opt, string filename, bool isSTDOUT, int nThreads){
    mOptions = opt;
    mNThreads = (nThreads > 0) ? nThreads : opt->thread;
    mWriter1 = NULL;
    ...
    if (mIsGzip && !mIsSTDOUT) {
        mNumCompressors = numCompressors(mOptions, mNThreads);
        startCompressPool();
    } else {
        mNumCompressors = 0;
    }
...
void WriterThread::output(){
    ...
    mWorkingBufferList = (mWorkingBufferList + 1) % mNThreads;
...
void WriterThread::initBufferLists() {
    mBufferLists = new SingleProducerSingleConsumerList<string*>*[mNThreads];
    for(int t=0; t<mNThreads; t++) {
