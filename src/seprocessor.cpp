...
bool SingleEndProcessor::processSingleEnd(ReadPack* pack, ThreadConfig* config){
    ...
    if(!filterResults.empty()) {
        for(size_t i = 0; i < readsForGPU.size(); i++) {
            ...
            // k-mer contamination scan on reads that pass the quality filter
            if (mContaminantDB && result == PASS_FILTER && r1 != NULL) {
...
void SingleEndProcessor::readerTask()
{
    ...
    while(true){
        ...
        if(count == mEffectivePackSize || needToBreak){
            ...
            // (threshold scales with thread count -- see packInMemLimit())
            while( mPackReadCounter - mPackProcessedCounter > packInMemLimit(mEffectiveThreads)){
                //cerr<<"sleep"<<endl;
                slept++;
                usleep(100);
            }
            ...
            if(readNum % (mEffectivePackSize * PACK_IN_MEM_LIMIT) == 0 && mLeftWriter) {
                while(mLeftWriter->bufferLength() > packInMemLimit(mEffectiveThreads)) {
                    slept++;
                    usleep(1000);
                }
            }
...
void SingleEndProcessor::processorTask(ThreadConfig* config)
{
    ...
    while(true) {
        ...
        if(input->isProducerFinished()) {
            ...
            // See PairEndProcessor::processorTask for why this replaces
            // usleep(100): blocking wakeup instead of busy-polling avoids
            // wake/sleep churn across many worker threads at high -w.
            input->waitForData(500);
        }
    }
