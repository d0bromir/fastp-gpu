...
int main(int argc, char* argv[]){
    ...
    // k-mer contamination detection
    cmd.add("detect_contaminants", 0, "enable k-mer (k=31) contamination detection. Built-in sources: TruSeq adapters, Nextera adapters. Use --contaminant_db to add custom FASTA sources.");
    cmd.add<string>("contaminant_db", 0, "path to a FASTA file with contaminant sequences to detect (in addition to built-ins). Only used when --detect_contaminants is specified.", false, "");
    cmd.add<int>("contaminant_min_hits", 0, "minimum k-mer hits required to call a read as contaminated (default 3, range 1-100).", false, 3);
    ...
    // k-mer contamination detection
    opt.contaminant.enabled     = cmd.exist("detect_contaminants");
    opt.contaminant.dbFile      = cmd.get<string>("contaminant_db");
    opt.contaminant.minKmerHits = cmd.get<int>("contaminant_min_hits");
    if (opt.contaminant.minKmerHits < 1)   opt.contaminant.minKmerHits = 1;
    if (opt.contaminant.minKmerHits > 100) opt.contaminant.minKmerHits = 100;
