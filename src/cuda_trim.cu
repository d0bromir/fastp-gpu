...
__global__ void trim_head_tail_kernel(
    ...
    // One thread per read: copy the whole trimmed range serially.  The
    // earlier cooperative form `for(i=threadIdx.x; i<new_len; i+=blockDim.x)`
    // assumed block-per-read parallelism, which is not how this kernel is
    // launched (idx already includes threadIdx.x), so it left bytes
    // uninitialised for any thread other than threadIdx.x == 0.
