...
The canonical benchmark + validation driver
([`scripts/run_benchmark.sh`](../scripts/run_benchmark.sh)) and the FASTQ
download helper ([`scripts/download_bench_samples.sh`](../scripts/download_bench_samples.sh))
live under [`scripts/`](../scripts/). On WSL hosts set
`FASTQ_DIR=/mnt/c/bio/FASTQ` before invoking; no separate WSL script.
